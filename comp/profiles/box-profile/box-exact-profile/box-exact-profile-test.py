...
first_time = True
try:
    disk_np_res = np.load("box-exact-prof-cH"+str(contour_H)+"-N"+str(N)+"-test.npy")
    first_time = False
...
for H_i, H in enumerate(hs):
    ...
    for j in range(0,stat_n//2):
        ...
        res2 = 0
        try:
            detect_H1 = fract.box_counting_H(z2d1, data=False)
            print("        ",detect_H1)
        ...
        try:
            detect_H2 = fract.box_counting_H(z2d2, data=False)
            # print("        ",res2)
        except Exception as ex:
            res2 = np.nan
            print("Exception ", ex)
...
# SAVING
np.save("box-exact-prof-cH"+str(contour_H)+"-N"+str(N)+"-test.npy", disk_np_res)

print(disk_np_res.shape)

%matplotlib tk

filename = "box-profile-response-contH-%g-N-%g.png"%(contour_H,N)
